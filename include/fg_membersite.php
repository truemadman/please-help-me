...
*/      
    ...
    // Function to search courses by course name
    function SearchCourse($qrystring)
    {
        if(!$this->DBLogin())
        {
            $this->HandleError("Database login failed!");
        ...
        $query = "SELECT maintable.name, maintable.email, courses.coursename, courses.hourrate FROM maintable INNER JOIN courses ON maintable.id_user=courses.ClientID where courses.coursename like '%". $qrystring . "%';"; //You don't need a ; like you do in SQL
        $result = mysql_query($query);
